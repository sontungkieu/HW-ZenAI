...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    ############################################################################
    if clip is None:
        raise ImportError("clip is required for CLIP zero-shot classification")
    ...
    pred_classes = [class_texts[idx] for idx in pred_indices.cpu().tolist()]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        ############################################################################
        if clip is None:
            raise ImportError("clip is required for CLIP image retrieval")
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        top_indices = similarity.squeeze(0).topk(k=min(k, len(self.images))).indices
...
class DavisDataset:
    def __init__(self):
        if tfds is None:
            raise ImportError("tensorflow-datasets is required for DavisDataset")
        if cv2 is None:
            raise ImportError("opencv-python is required for DavisDataset")
