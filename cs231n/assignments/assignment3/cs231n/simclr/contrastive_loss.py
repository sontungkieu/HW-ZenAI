...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        total_loss += -torch.log(numerator_left / denominator_left)
        total_loss += -torch.log(numerator_right / denominator_right)
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    mask = (
        torch.ones_like(exponential, device=exponential.device)
        - torch.eye(2 * N, device=exponential.device)
    ).bool()
